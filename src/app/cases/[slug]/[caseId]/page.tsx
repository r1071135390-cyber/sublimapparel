...
/**
 * 2026-09-12 (R50): extract the delivery region from a case study
 * summary string. Case summaries in src/lib/cases.ts end with a
 * comma-separated "Region, Country" phrase (e.g. "Ohio, United
 * States", "Midwestern United States", "Lake County, United
 * States"). The function pulls the last comma-pair and returns
 * the trimmed result, or null if no match is found. This feeds
 * the Report.spatialCoverage field on the case detail Article
 * node so Google can ground each case study to a geographic
 * region — the most decision-critical filter for B2B apparel
 * buyers comparing vendors by region.
 */
function extractLocation(summary: string): string | null {
  // 2026-09-12 (R50): only match the US case summaries today
  // (the only country in cases.ts is the US). When case
  // coverage expands to EU/UK/APAC, extend this regex to
  // accept those country names.
  const m = summary.match(/([A-Z][A-Za-z .'-]+?), United States\.$/);
...
export default async function CaseDetailPage({ params }: Props) {
  ...
  // 2026-09-12 (R50): add HowTo as a 4th sibling node on every case
  // study detail page. The case summary is a single paragraph, not
  // a step-by-step, but the underlying production flow is universal
  // across all our case studies (brief → sample → bulk → QC → ship).
  // Synthesizing a 5-step HowTo from that universal flow gives Google
  // 8 new pages eligible for the HowTo rich result on queries like
  // "how to deliver a custom apparel case study" or "how does B2B
  // apparel production work". buildHowToNode reuses the R48 helper
  // for entity graph cross-linking (isPartOf → #webpage, about →
  // #organization, inLanguage: "en", url) so the case study HowTo
  // is consistent with /quality-control/, /production/ and the
  // technique detail HowTo.
  const howToId = `${caseUrl}#howto`;
  const howToNode = buildHowToNode({
    howToId,
    webpageId: `${caseUrl}#webpage`,
    name: `How we delivered the ${c.title.split(" — ")[0]} project`,
    description: `Standard 5-step B2B apparel delivery we used for this ${ind.title.toLowerCase()} case: brief lock, lab sample, bulk production, AQL inspection, DDP/FOB ship.`,
    totalTime: "P60D",
    steps: [
      {
        name: "Step 1: Brief & artwork lock",
        text: "We lock the design brief, fabric spec, sizing, and quantities. Pantone colors, print placement, and any special finishes are documented in a one-page spec sheet signed by the buyer.",
      },
      {
        name: "Step 2: Lab sample & client sign-off",
        text: "We produce a pre-production sample on the exact fabric. The buyer reviews color, sizing, print fidelity, and finishing. Sign-off is the contract for bulk.",
      },
      {
        name: "Step 3: Bulk production",
        text: "Bulk fabric is cut, printed or embroidered, sewn, and finished on the same Yiwu production floor. Mid-production photo checks go out to the buyer at the 30% and 70% completion milestones.",
      },
      {
        name: "Step 4: AQL 2.5 quality control",
        text: "Statistical sample inspection at the final stage. Defects graded Critical / Major / Minor per ISO 2859-1. Failed lots are 100% re-inspected before pack-out.",
      },
      {
        name: "Step 5: Pack, ship & deliver DDP",
        text: "Packed per buyer spec, poly-bagged, labeled, and shipped DDP or FOB. Photo evidence of the finished, packed, and loaded cartons is sent before the container is sealed.",
      },
    ],
  });
  ...
  const caseGraph = {
    ...
    "@graph": [
      ...
      // 3 · Article + Report (case study content node) — R50:
      // upgrade @type from "Article" to ["Article", "Report"]. Case
      // studies are data-driven case reports, not editorial articles;
      // Report is Google's recognized schema for "a document with
      // structured findings about a specific topic". The multi-@type
      // array is the canonical pattern — Google reads both, and Report
      // gets richer AI Overview treatment for "X case study" or
      // "example of Y" queries. spatialCoverage lets Google ground
      // the case study to a geographic region (already parsed from
      // c.summary) which is the most decision-critical filter for
      // B2B apparel buyers comparing vendors by region.
      {
        "@type": ["Article", "Report"],
        "@id": articleId,
        headline: c.title,
        ...
        // 2026-09-12 (R50): Report-specific signals.
        // reportNumber is a stable per-case identifier (case id)
        // so Google can deduplicate case studies across the site
        // and across the wider web. spatialCoverage is the
        // geographic region the project was delivered to —
        // parsed from c.summary (e.g. "Ohio, United States"),
        // and falls back to the parent industry region if the
        // summary doesn't surface a specific location.
        reportNumber: c.id,
        spatialCoverage: extractLocation(c.summary) ?? ind.title,
        url: caseUrl,
      },
      // 4 · HowTo (R50) — universal 5-step B2B apparel delivery
      // process. Synthesized from the case summary's underlying
      // production flow. Same entity graph pattern as the
      // technique detail pages: isPartOf → #webpage + about →
      // #organization so Google can trace the steps back to the
      // case study that hosts them.
      howToNode,
    ],
  };
