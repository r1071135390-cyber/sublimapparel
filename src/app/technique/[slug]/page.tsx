...
export default async function TechniqueDetailPage({ params }: PageProps) {
  ...
  // step list is parseable as a Google HowTo rich result. HowTo joins
  // the graph via its @id so Google can pick it up for how-to queries.
  // 2026-09-12 (R50): with the HowTo now first-class on every
  // technique page, mainEntity shifts from FAQPage → HowTo. The
  // page's primary purpose is to teach a decoration process; the FAQ
  // is supporting content. The FAQPage node stays in the graph as a
  // sibling for PAA eligibility, but the WebPage now declares
  // HowTo as the page's primary entity, which is the correct
  // semantic for "how does X printing work" queries.
  const techniqueGraph = {
    ...
    "@graph": [
      ...
      // 2 · Article (the technique guide content node) — R50:
  // upgrade @type from "Article" to ["Article", "TechArticle"]
  // so Google classifies the page as technical content, not
  // just a generic blog/news article. TechArticle is Google's
  // recognized sub-type for technical how-to articles and
  // explicitly supports the same fields we already populate
  // (headline, description, inLanguage, author, publisher,
  // keywords). The multi-@type array is the canonical
  // pattern — Google reads both, and downstream consumers
  // (PAA, AI Overviews) prefer TechArticle for technical
  // queries. proficiencyLevel + dependencies are the two
  // TechArticle-specific fields that make the classification
  // unambiguous to the parser.
      {
        ...
        // 2026-09-12 (R50): TechArticle-specific signals.
        // proficiencyLevel tells Google the technical depth
        // the article assumes of the reader. "Beginner" is
        // correct for our technique pages because they read
        // like onboarding guides for B2B buyers who are
        // often first-time importers of this print method.
        // dependencies lists the upstream skill/material
        // required to execute the technique — pattern making,
        // artwork separation, fabric sourcing — so Google
        // can connect this page to related content in the
        // entity graph (e.g. /technique/ hub and /fabric/).
        proficiencyLevel: "Beginner",
        dependencies: [
          "Apparel pattern (cut & sew template)",
          "Artwork file (vector or 300+ DPI raster)",
          "Base fabric compatible with this decoration method",
        ],
        url: techUrl,
      },
      // 3 · BreadcrumbList
