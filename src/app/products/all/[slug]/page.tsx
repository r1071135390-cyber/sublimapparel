...
import { buildProductDetailGraph } from "@/lib/breadcrumb";
import {
...
export default async function ProductDetailPage({
  ...
  //
  // 2026-09-12 (R35-B): consolidate the previous 3 independent
  // <script> tags (BreadcrumbList + flat Product + FAQPage) into a
  // single @graph block via buildProductDetailGraph. The product +
  // review + aggregateRating + offer payload built below is fed
  // straight into the helper; the helper wraps it with WebPage,
  // Service, BreadcrumbList, and FAQPage siblings, all joined via
  // @id so Google parses the entire entity surface in one pass.
  ...
  // The 6 FAQ items live in seo.faq (per-product, generated by
  // buildSeoContent). Map the {question, answer} shape to the
  // {q, a} FaqItem shape the shared helper expects.
  const productFaqSchema = seo.faq.map((q) => ({ q: q.question, a: q.answer }));

  const productGraph = buildProductDetailGraph({
    slug: product.slug,
    name: product.name,
    description: seo.metaDescription,
